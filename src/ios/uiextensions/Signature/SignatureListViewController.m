...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        __weak SignatureListCell* weakSelf = self;
        self.editItem = [TbBaseItem createItemWithImageAndTitle:NSLocalizedString(@"kEdit", nil) imageNormal:[UIImage imageNamed:@"signEdit"] imageSelected:[UIImage imageNamed:@"signEdit"] imageDisable:[UIImage imageNamed:@"signEdit"] background:nil imageTextRelation:RELATION_BOTTOM];
        ...
        self.deleteItem = [TbBaseItem createItemWithImageAndTitle:NSLocalizedString(@"kDelete", nil) imageNormal:[UIImage imageNamed:@"signDelete"] imageSelected:[UIImage imageNamed:@"signDelete"] imageDisable:[UIImage imageNamed:@"signDelete"] background:nil imageTextRelation:RELATION_BOTTOM];
...
{
    self.tableView = nil;
    self.signatureArray = nil;
    self.currentName = nil;
    self.selectSignatureHandler = nil;
    self.openSignatureViewHandler = nil;
    self.cancelSignatureHandler = nil;
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (DEVICE_iPHONE || SIZECLASS == UIUserInterfaceSizeClassCompact) {
        __weak SignatureListViewController* weakSelf = self;
        ...
        self.cancelItem.onTapClick = ^(TbBaseItem *item)
        {
            [weakSelf dismissViewControllerAnimated:YES completion:^{
                if (weakSelf.cancelSignatureHandler) {
                    weakSelf.cancelSignatureHandler();
                }
            }];
        };
    ...
    __weak SignatureListViewController* weakSelf = self;
    self.createItem = [TbBaseItem createItemWithTitle:NSLocalizedString(@"kCreate", nil)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        if (_selectSignatureHandler)
        {
            _selectSignatureHandler();
        }
    }];
}

#pragma mark -  table view datasource handler
//static const int CELL_BACKGROUND_INSET = 10;
//static const int CELL_IMAGE_INSET = 20;
//static const int CELL_IMAGE_INSET_IPHONE = 25;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.moreShowClick = ^(SignatureListCell *editCell){
        ...
        SignatureListCell *oldEditCell = [tableView cellForRowAtIndexPath:self.oldIndexPath];
        if (oldEditCell) {
            oldEditCell.showMoreContentView = NO;
        }
        editCell.showMoreContentView = YES;
        self.oldIndexPath = [tableView indexPathForCell:editCell];
    };
    cell.editItemClick = ^(SignatureListCell *editCell){
        ...
        if (DEVICE_iPHONE) {
            [self dismissViewControllerAnimated:NO completion:^{
                NSString *name;
                if (self.isFieldSigList) {
                    name = [self.signatureArray objectAtIndex:[tableView indexPathForCell:editCell].row];
                }else{
                    name = [[self.signatureArray objectAtIndex:[tableView indexPathForCell:editCell].section] objectAtIndex:[tableView indexPathForCell:editCell].row];
                }
                AnnotationSignature *sig = [AnnotationSignature getSignature:name];
                if (_openSignatureViewHandler)
                {
                    _openSignatureViewHandler(sig);
                }
            }];
        }
        else {
    ...
    cell.deleteItemClick = ^(SignatureListCell *editCell){
        ...
        [self.tableView reloadData];
        if (_deleteSignatureHandler) {
            _deleteSignatureHandler(sig);
        }
    };
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (section == DIGITALSIGNATUREGROUP) {
        label.text = NSLocalizedString(@"kSignListSectionHadCert", nil);
    }else{
        label.text = NSLocalizedString(@"kSignListSectionWithoutCert", nil);
    }
