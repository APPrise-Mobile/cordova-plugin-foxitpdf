...
- (void)initSubView
{
    ...
    SignatureViewController* __weak weakSelf = self;
    ...
    if (self.currentCertFileName) {
        NSString *title = [NSLocalizedString(@"kCurrentSelectCert", nil) stringByAppendingString:self.currentCertFileName];
        [self.createCertBtn setTitle:title forState:UIControlStateNormal];
    }else{
        [self.createCertBtn setTitle:NSLocalizedString(@"kSelectAddCert", nil) forState:UIControlStateNormal];
    }
...
- (void)selectCert
{
    ...
    digitalCertSelectCtr.title = NSLocalizedString(@"kSelectAddCert", nil);
    ...
    digitalCertSelectCtr.doneOperator = ^(NSString *path, NSString *passwd, NSString *md5){
        NSString *title = [NSLocalizedString(@"kCurrentSelectCert", nil) stringByAppendingString:[path lastPathComponent]];
