...
- (BOOL)onPageViewTap:(int)pageIndex recognizer:(UITapGestureRecognizer *)recognizer
{
    ...
    if (!_isAdded)
    {
        ...
        MenuItem *signItem = [[MenuItem alloc] initWithTitle:NSLocalizedString(@"kSignAction", nil) object:self action:@selector(sign)];
...
- (void)addSignAsStamp
{
...

    {
        ...
        if (signAnnot.signature )//ret == FSCRT_ERRCODE_SUCCESS && annotPtr)
        {
            //Set name (uuid)
...
-(void)signList
{
    SignToolHandler* __weak weakSelf = self;
    UIViewController *rootViewController = [UIApplication sharedApplication].keyWindow.rootViewController;
    ...
    __weak UIExtensionsManager* extMgrWeak = _extensionsManager;
    __weak FSPDFViewCtrl* pdfViewCtrlWeak = _pdfViewCtrl;
    self.signatureListCtr.deleteSignatureHandler = ^(AnnotationSignature *sig)
    {
        UIImage *image = [AnnotationSignature getSignatureImage:weakSelf.signature.name];
        image = [Utility scaleToSize:image size:CGSizeMake(image.size.width/2, image.size.height/2)];
        if (!image) {
            _isAdded = NO;
            if (extMgrWeak.menuControl.isMenuVisible)
            {
                [extMgrWeak.menuControl setMenuVisible:NO animated:YES];
            }
            weakSelf.annotImage = nil;
            CGRect newRect = [pdfViewCtrlWeak convertPdfRectToPageViewRect:weakSelf.signature.rect pageIndex:weakSelf.signature.pageIndex];
                newRect = CGRectInset(newRect, -30, -30);
            [pdfViewCtrlWeak refresh:newRect pageIndex:weakSelf.signature.pageIndex needRender:NO];
        }
        else
        {
            CGRect newRect = [pdfViewCtrlWeak convertPdfRectToPageViewRect:weakSelf.signature.rect pageIndex:weakSelf.signature.pageIndex];
    ...
    self.signatureListCtr.cancelSignatureHandler = ^()
    {
        if (DEVICE_iPHONE) {
            ...
            if ([AnnotationSignature getSignatureList].count <= 0)
            {
                [extMgrWeak setCurrentToolHandler:nil];
            }
        }
    };
    ...
    self.signatureListCtr.openSignatureViewHandler = ^(AnnotationSignature *sig)
    {
        ...
        SignatureViewController *signatureCtr = [[SignatureViewController alloc] initWithUIExtensionsManager:extMgrWeak];
        signatureCtr.modalPresentationStyle = UIModalPresentationOverFullScreen;
